class Api::V1::TeacherProfilesController < BaseController

    def update
        result = TeacherProfiles::UpdateTeacherProfileFlow.call(params: teacher_profile_params,
                                                              id: params[:id])

        if result.success?
            serialized_teacher_profile = TeacherProfileSerializer.new.serialize(result.profile)
            
            render json: { profile: serialized_teacher_profile }, status: :ok
        else
